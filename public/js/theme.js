    ...
    var LockScreen = {
        ...
        buildTemplate: function( userinfo ) {
            return [
                ...
                '<form>',
                '<div class="current-admin text-center">',
                '<img id="LockUserPicture" src="{{picture}}" alt="John Doe" class="img-circle admin-image" />',
                '<h2 id="LockUserName" class="admin-name text-dark m-none">{{username}}</h2>',
                '<p  id="LockUserEmail" class="admin-email m-none">{{email}}</p>',
                '</div>',
